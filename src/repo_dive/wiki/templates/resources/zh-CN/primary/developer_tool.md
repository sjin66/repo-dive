...
<!-- repo-dive:page=tool_diagnostics_page; order=5; cardinality=1; shape=heading,paragraph,table,code_block; purpose=说明诊断与故障排查; evidence=必需证据包括错误、日志、检查、调试命令和单基行号; constraints=把症状映射到有证据的原因与安全恢复; -->
**{{repo_dive:tool_diagnostics_page}}**：列出可观察失败、诊断、修复和升级证据。
<!-- repo-dive:page=tool_distribution_page; order=6; cardinality=1; shape=heading,paragraph,list,code_block; purpose=说明测试、打包与分发; evidence=必需证据包括测试、包元数据、发布自动化和单基行号; constraints=平台、产物与安装渠道必须有依据; -->

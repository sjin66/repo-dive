...
## Planned RAG Command Boundary

The planned command families expose each RAG stage independently:
...
`index`, `search`, and `context` are deterministic RAG operations. `wiki` accepts content generated by the calling Copilot session; none of these commands implicitly calls a generative model. These names describe the approved interface direction and are not available in the foundation build.

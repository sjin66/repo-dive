...
The current build implements `index`, `search`, and `context` in addition to `--help` and `--version`.
...
`index`, `search`, and `context` are available deterministic RAG operations. The planned `wiki` family will accept content generated by the calling Copilot session. None of these commands implicitly calls a generative model.
